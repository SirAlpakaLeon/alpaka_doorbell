            ...
            function insideZone(self)
                ...
                if time <= 0 then
                    ...
                    if IsControlJustReleased(0,  38) then
                        TriggerServerEvent('alpaka_doorbell:NotifyJob', e.job)
                        lib.notify({
                            title = Config.YouRang,
                            type = 'inform'
                        })
                        time = Config.WaitingTime
                        TriggerEvent("alpaka_doorbell:startwaittime")
                    end
...
RegisterNetEvent("alpaka_doorbell:startwaittime")
AddEventHandler("alpaka_doorbell:startwaittime", function()
    while time >= 0 do
        Citizen.Wait(1000)
        time = time - 1
    end
